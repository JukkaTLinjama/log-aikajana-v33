...
(() => {
    ...
    function layout() {
        ...
        zoomBG.attr("x", 0).attr("y", 0).attr("width", cfg.zoomBar.width).attr("height", innerH - 1); // -1px: ei ylitystä
    ...
    function updateZoomIndicator(transform) {
        ...
        const k = Math.max(transform.k, 1e-6);
        const ty = transform.y;

        // ikkuna-korkeus: clampaa [MIN_H, H]
        const MIN_H = 8;
        const rawH = H / k;
        const winH = Math.max(MIN_H, Math.min(H, rawH));

        // sijainti: clampaa niin että winY + winH ≤ H
        let winY = -ty / k;
        winY = Math.max(0, Math.min(winY, H - winH));
